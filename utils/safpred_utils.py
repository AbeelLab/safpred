...
def parse_database_file(db_path, emb_path, keep_clusters=None, keep_singletons=False, avg_synteny=False):
    ...
    add_region = [{'region': tuple([cnum]), 'intergenic_dist': [0], 'region_len': 1} 
                  for cnum in add_clusters]
    ...
    if avg_synteny:
        ...
        cluster_emb_matrix = cluster_emb_matrix/np.linalg.norm(cluster_emb_matrix, 
        ...
        for rnum, row in new_db_df.iterrows():
            if row.region_len == 1: # only 1 cluster in the region
                ...
                emb_vec = np.array([cluster_emb_matrix[cluster2row[cnum], :] 
                                    for cnum in row.region])
...
def predict_from_avg_synteny(cluster2go, db_df, nn_dict):
    ...
    for i, (query_id, nn) in enumerate(nn_dict.items()):
        ...
        for rsim, rnum in nn:
            for go, go_freq in region2gofreq[rnum].items():
                if go not in go_pred.keys():
                    go_pred[go] = float(go_freq*rsim) # scale go term freq with the similarity
                else:
                    go_pred[go] = max(go_pred[go], float(go_freq*rsim))
